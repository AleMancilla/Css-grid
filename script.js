...
const timerUpdate=setInterval(()=>{/*Crear la función que se va actualizar cada segundo*/
    ...
    document.getElementById("days").innerHTML = `${t.remainDays} <span>Días</span>`; 
    document.getElementById("hours").innerHTML = `${t.remainHours} <span>Horas</span>`; 
    document.getElementById("minutes").innerHTML = `${t.remainMinutes} <span>Minutos</span>`; 
    document.getElementById("seconds").innerHTML = `${t.remainSeconds} <span>Segundos</span>`; 
...
countdown("Oct 17 2021 15:26:43 GMT-0600","clock","Muchas Gracias");